...
export class TimeOffTicket extends Ticket {
    ...
    constructor(employee: User, employer: User, startDate: Date, endDate: Date) {
        super("Time Off Request", TicketType.TimeOff, 2,
            `${employee.displayName} is requesting that ${employer.displayName} grants them time off from ${startDate} to ${endDate}`);
    ...
    approve(message?: string) {
        ...
        if (message) {
            this.issue += `\n${this.employer.displayName} has approved your request for the following reason: ${message}`;
        }
    }
    ...
    deny(message?: string) {
        ...
        if (message) {
            this.issue += `\n${this.employer.displayName} has denied your request for the following reason: ${message}`;
        }
    }

    getTimeOffIssue(): string {
        return `${this.employee.displayName} is requesting that ${this.employer.displayName} grants them time off from ${this.startDate} to ${this.endDate}`;
    }
